...
RUSTFLAGS_VAR="CARGO_TARGET_${UUTRIPLE}_RUSTFLAGS"
RUSTFLAGS_VALUE=
if [[ -v $RUSTFLAGS_VAR ]]; then
	RUSTFLAGS_VALUE="${!RUSTFLAGS_VAR}"
fi
export "$RUSTFLAGS_VAR=${RUSTFLAGS_VALUE:+$RUSTFLAGS_VALUE }-C link-arg=-Wl,--pack-dyn-relocs=android"
